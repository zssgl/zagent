...
| 客单价（严格口径） | 一人一天在一家门店所有消费合并后计算 | 待定义：按顾客合并日内所有单据，再算人均 |
...
| 目标缺口预测 | 基于剩余天数/预约预测缺口 | 待定义：预测模型与预约清单字段 |
...
| 指标 | 业务口径 | 字段口径（表/字段/过滤/计算） |
| --- | --- | --- |
| 老带新人数/GMV | 客户来源为会员推荐的新增客 | 待接入：推荐关系表/字段与新客判断 |
| 美丽基金核验 | 检查基金是否添加且金额正确 | 待接入：基金表与排除规则（折扣码/套餐/内卖） |
| 渠道贡献拆分 | 新客渠道对生意增减的贡献 | 待定义：贡献拆分口径与对比窗口 |
| 单项目/ VIP 比例 | 单项目或 VIP 占比 | 待定义：分子/分母一致口径（到店或开单顾客） |

#### 关键品项完成 - 额外口径

| 指标 | 业务口径 | 字段口径（表/字段/过滤/计算） |
| --- | --- | --- |
| 品项人数 | 当月品项去重顾客数 | 待接入：品项-顾客明细 |
| WOW/同期对比 | 对比上月同期或近三月同期 | 待定义：同比/环比窗口与基准 |
| 单次比例 | 单次消费顾客占比 | 待定义：单次顾客口径 |
| 疗程剩余/需复购 | 有未消耗疗程顾客占比 | 待接入：疗程/次数表 |
...
| 病历/处方明细 | 未完成名单与具体记录 | 待接入：病历/处方明细关联 |
...
| VVIP活跃度 | VVIP到店/活跃程度 | 待接入：VVIP标签与活跃口径 |
...
| 预约分群 | VVIP/复购周期/需开单/生日福利等 | 待接入：顾客标签、复购周期规则 |
| 排班与人手风险 | 当班医生/护士与人手缺口 | 待接入：排班表与风险规则 |
...
| 未来7天目标与预约 | 未来7天目标与预约量 | 待接入：日目标与预约计划表 |

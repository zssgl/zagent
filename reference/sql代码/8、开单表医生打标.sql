...
DROP TABLE IF EXISTS report_bill_doctor;
CREATE TABLE report_bill_doctor (
    开单门店 VARCHAR(255),
    病历编号 VARCHAR(255),
    顾客姓名 VARCHAR(255),
    `项目ID` VARCHAR(255),
...
-- Step 2: Insert data
INSERT INTO report_bill_doctor
SELECT * FROM (
    SELECT 开单门店, 病历编号, 顾客姓名,`项目ID`, 开单项目,数量, 客户来源,
        付款时间, 最早开单时间, 最后消耗时间, `订单来源`,`是否是内购订单`,开单时间, 医生业绩, doc_count,
